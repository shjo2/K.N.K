...
void analyze_hand(int hand[RANK][SUIT], bool* royalstraight, bool* straight, bool* flush, bool* four, bool* three, int* pairs);
void print_result(bool royalstaight, bool straight, bool flush, bool four, bool three, int pairs);
bool is_duplicated(int hand[RANK][SUIT], int rank, int suit, int cards_read);

int main(void) {
    int hand[RANK][SUIT];

    bool royalstraight, straight, flush, A, five, four, three;
    int pairs;

    for (;;) {
        read_cards(hand);
        analyze_hand(hand, &royalstraight, &straight, &flush, &four, &three, &pairs);
...
void analyze_hand(int hand[RANK][SUIT], bool* royalstraight, bool* straight, bool* flush, bool* four, bool* three, int* pairs) {
    ...
    *pairs = 0;
    ...
    // check straight
    int rank = 0;
    while (rank_count[rank] == 0) rank++;
    for (; rank < NUM_RANKS && rank_count[rank] > 0; rank++)
        num_consec++;
...
void print_result(bool royalstraight, bool straight, bool flush, bool four, bool three, int pairs) {

...
void generate_radom_walk(char randomWalk[WALK_SIZE][WALK_SIZE]) {
	int row = 0;
	int col = 0;
	int next_row = 0;
	int next_col = 0;
...
void print_array(char randomWalk[WALK_SIZE][WALK_SIZE]) {
	for (int i = 0; i < WALK_SIZE; i++) {
		for (int j = 0; j < WALK_SIZE; j++) {
...
int main(void)
{
	...
	generate_radom_walk(randomWalk);
	printf("Random Walk: \n");
	print_array(randomWalk);
